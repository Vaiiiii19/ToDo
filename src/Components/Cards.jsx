    ...
    >
      ...
      <div className="footer absolute bottom-0  w-full   left-0">
        <div className="flex items-center justify-between py-3 px-8 mb-3">
          <h5>{data.filesize}</h5>
          <span className="w-7 h-7 bg-zinc-600 rounded-full flex item-center justify-center">
            {data.close ? (
              <IoClose />
            ) : (
              <AiOutlineDownload size="1em" color="#fff" />
            )}
          </span>
        </div>
        {data.tags.isOpen && (
          <div
            className={`tag w-full py-4 ${
              data.tags.tagColor === "blue" ? "bg-blue-600" : "bg-green-600"
